...
{{< lead >}}
**Sigil Gate** — приватная сеть с архитектурой core-periphery, устойчивая к обнаружению системами Deep Packet Inspection (DPI) и активному зондированию.
{{< /lead >}}
...
- устойчива к обнаружению системами DPI и активному зондированию;
...
## Архитектура core-periphery

Сеть **Sigil Gate** построена по модели **core-periphery** (ядро-периферия) — плотно связанное ядро из Core-нод и периферия из Entry-нод, подключенных к ядру.

Система состоит из четырех типов сущностей:

| Сущность | Роль | Плоскость |
|----------|------|-----------|
| [Root-нода]({{< ref "docs/nodes/root" >}}) | Центр управления, PKI, развертывание | Управление |
| [Core-нода]({{< ref "docs/nodes/core" >}}) | Ядро сети, маршрутизация трафика в интернет | Данные + управление |
| [Entry-нода]({{< ref "docs/nodes/entry" >}}) | Входной узел, прием клиентских подключений | Данные |
| [Клиент]({{< ref "docs/nodes/client" >}}) | Устройство пользователя | Данные |
...
Это образует древовидную структуру (hub-and-spoke) в плоскости управления:
...
Административная привязка — условная. Для передачи данных Entry-нода может быть связана с **несколькими** Core-нодами, но не более чем с **третью** от общего числа Core-нод. Это ограничение реализует принцип минимальной связности: при компрометации Entry-ноды раскрывается лишь малая часть ядра сети.
...
{{< alert >}}
Механизмы динамического перераспределения Entry-нод между Core-нодами находятся на этапе архитектурного планирования и пока не реализованы.
{{< /alert >}}
...
При обращении на любой URL без правильных VLESS-заголовков сервер возвращает сайт-прикрытие. Наблюдатель не может отличить «секретный путь» от «обычной страницы» — нет характерных ошибок, редиректов или отказов.
...
gRPC выбран как единственный транспорт:
...
{{< alert >}}
Под **компрометацией** ноды понимается обнаружение и блокировка системами DPI, а не несанкционированный доступ или физическое проникновение. Признаки компрометации — потеря HTTPS-связности между Entry- и Core-нодами при сохранении SSH-доступа, или существенная деградация канала связи.
{{< /alert >}}
...
Каждая нода плоскости данных (Entry и Core) размещает полноценный веб-сайт:

- тип: SPA (Single Page Application);
- размер: >16 КБ (для корректной работы при частичной загрузке);
- контент: уникальный, нейтральный;
- требование: сайт должен выглядеть легитимным и «живым» при ручной проверке.

Root-нода не имеет сайта-прикрытия, так как не участвует в сетевом трафике.
