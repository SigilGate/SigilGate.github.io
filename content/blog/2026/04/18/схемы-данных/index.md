...
draft: false
summary: "etcd-кластер запущен и ждет данных. Рассказываю, почему перенести всё как есть не получилось — и как миграция превратилась в полноценный пересмотр схем."
...
Первая мысль, которая приходит в голову в такой ситуации — не мудрить. Взять то, что есть, перенести как есть, а потом разберёмся по месту: добавим полей, уберём лишних, подгоним под новые требования. Звучит разумно — и именно поэтому я не так быстро от этой идеи отказался.

---

В старой архитектуре данные хранятся в виде JSON-файлов: одна папка на тип сущности, один файл на каждый экземпляр. Пользователь — `users/1.json`. Устройство — `devices/<uuid>.json`. Маршрут — `routes/entry_195.2.67.202_core_45.63.121.41.json`. И так далее.
...
Миграция — это не просто смена формата хранения. Это повод остановиться и спросить.

Я немного почистил данные перед переездом. Убрал поля, которые закладывались «на будущее», до которого дело так и не дошло. Убрал то, что в какой-то момент стало устаревшим, когда поменялась схема. Убрал аннотации и служебные артефакты, которые не несут функциональной нагрузки. Хлам есть в любой живой системе — лучше его не тащить в новый дом.
...
**Операционный слой.** Всё, что нужно для работы сети: ноды, маршруты, пользователи, устройства. Только обезличенные данные — никаких контактов, никаких явных идентификаторов. Telegram ID больше не хранится в явном виде — только его хеш, нужный для корреляции с коммуникационным слоем в памяти приложения. Прямой связи между слоями в базе нет.
...
Попутно написал и backup-сервис. Схема та, что и планировалась изначально: etcd — основное хранилище, Git-репозиторий — резервная копия. Периодический dump из etcd в JSON, коммит в репозиторий. Если кластер когда-нибудь придётся поднимать с нуля — данные никуда не денутся.
