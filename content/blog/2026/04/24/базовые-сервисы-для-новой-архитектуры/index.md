...
draft: false
summary: "Управляющий кластер развернут, данные переехали, API задеплоен — и тут появляется список. Длинный. Рассказываю, как из этого списка выбрать только то, без чего сеть физически не запустить."
categories: ["Дневник проекта"]
rubrics: ["Хроники проекта"]
tags: ["api", "архитектура-2.0", "этап-3"]
---

Управляющий кластер развернут. Данные [перебрались на новое место]({{< ref "blog/2026/04/20/новые-жильцы" >}}). API задеплоен и отвечает по HTTPS. Казалось бы — всё готово. Бери и запускай сеть.

И тут в голове появляется список.

Длинный такой список. Архитектура 2.0 — это не просто «API вместо скриптов». Это полноценная операционная система для сети: полный CRUD по пользователям и устройствам, ротация нод, механизм апелляций, CLI оператора, новый Telegram-бот, периодические снапшоты состояния. И это не считая входного кластера и перевода Core-нод на схему phone-home.

Хочется всего и сразу. Скарлетт О'Хара обещала себе подумать об этом завтра — я же просто составил список. Но список от реальности не отличишь, пока в него не упрёшься.

Реальность простая: чтобы сеть заработала хотя бы в минимальном виде, нужно не «всё и сразу», а ровно тот минимум, без которого физически не включить ни одного клиента.
...
Семь сервисов написаны. 151 тест зелёный. Пуш в кластер.

Первая же грубая проверка в условиях, приближенных к боевым, выявила три бага.
...
Доработано, протестировано, задеплоено.
