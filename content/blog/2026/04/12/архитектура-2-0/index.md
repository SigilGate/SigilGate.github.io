...
draft: false
summary: "Атака криптомайнера стала поводом — но не причиной. Разбираю, что не так было в старой архитектуре и как я её пересмотрел: от дерева к двум независимым кластерам, нулевое доверие, один домен на кластер и онбординг через Docker."
...
Решает: жесткую топологию, бутылочные горлышки при масштабировании.
...
**Вступление через Docker.**

Участник устанавливает Docker, получает одноразовый join-токен, запускает контейнер. Контейнер генерирует ключевую пару локально, отправляет CSR на управляющий кластер, получает сертификат и регистрируется в сети. Приватный ключ не покидает сервер участника. SSH-доступа к серверу я не получаю.

Решает: операционную сложность онбординга; подключение нового участника — это два контейнера и один токен.

**Выделенный управляющий кластер.**

Три сервера с Kubernetes control plane, HA etcd и Root CA — отдельно от плоскости данных. Здесь же сервисная инфраструктура: Telegram-бот, резервное копирование, subscription-сервис. Управляющий кластер не участвует в передаче трафика — только управление и PKI.

Решает: единую точку отказа; отказоустойчивость за счет трех узлов с кворумом.
...
Подробная техническая документация — в [разделе архитектуры](https://sigilgate.github.io/docs/).
